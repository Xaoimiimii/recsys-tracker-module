...
export class PayloadBuilder {
    ...
    /**
     * NEW: Check xem rule có field nào cần chờ INTERCEPTOR (requestbody/responsebody) không
     * RequestUrl không cần chờ vì extract từ history
     */
    needsNetworkInterceptor(rule) {
        if (!rule.payloadMappings)
            return false;
        return rule.payloadMappings.some((m) => {
            const source = (m.source || '').toLowerCase();
     ...
     * - LocalStorage, Cookie, SessionStorage, Element, Url: extract ngay
     * - RequestUrl: extract ngay từ history (không cần chờ interceptor)
     * - RequestBody/ResponseBody: skip, sẽ được xử lý bởi NetworkExtractor
     */
    collectNonNetworkData(pending) {
        ...
        for (const mapping of pending.rule.payloadMappings) {
            const source = (mapping.source || '').toLowerCase();
            // Skip ONLY request/response body (cần chờ NetworkExtractor)
            // RequestUrl có thể extract ngay từ history
            if ([
                'requestbody',
                'responsebody',
                'request_body',
                ...
                console.log('[PayloadBuilder] Skipping body source (will wait for interceptor):', source, 'for field:', mapping.field);
                continue;
            }
            // Thu thập data từ all other sources (bao gồm RequestUrl)
            const extractor = this.extractors.get(source);
            if (extractor) {
                ...
                if (this.isValid(val)) {
                    ...
                    console.log('[PayloadBuilder] Could not extract:', mapping.field, 'from source:', source);
                }
            }
        }
    }
    ...
    checkAndComplete(ruleId) {
        ...
        // Check xem có cần chờ network interceptor không (chỉ requestbody/responsebody)
        const needsInterceptor = this.needsNetworkInterceptor(pending.rule);
        if (needsInterceptor && !pending.networkCaptured) {
            console.log('[PayloadBuilder] Waiting for network interceptor (requestbody/responsebody)...');
        ...
        // Nếu đã đủ dữ liệu hoặc không cần chờ interceptor → Complete
        if (missingFields.length === 0) {
            console.log('[PayloadBuilder] ✅ All fields collected, completing...');
            this.completePendingCollection(ruleId, false);
        }
        else if (pending.networkCaptured) {
            console.log('[PayloadBuilder] ⚠️ Network captured but still missing fields, completing anyway...');
            this.completePendingCollection(ruleId, false);
        }
        else {
            console.log('[PayloadBuilder] ⏳ Still missing fields but no interceptor needed, might complete later...');
        }
    }
    /**
...
//# sourceMappingURL=payload-builder.legacy.js.map